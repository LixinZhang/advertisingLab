...
def dataCleaning() :
    ...
    dumpDict2File(ad2Users, TMP_DATA_DIR_PATH + 'ad2UsersGivenAdSet_test.dict')
    userDict = generateUser2AdGivenAd2User(TMP_DATA_DIR_PATH + 'ad2UsersGivenAdSet_test.dict', adViewThreshold = 1)
    dumpDict2File(userDict, TMP_DATA_DIR_PATH + 'user2AdGivenAd2User_test.dict')
    userSet = set()
    #logging.debug(len(userSet))
    for line in file(TMP_DATA_DIR_PATH + 'user2AdGivenAd2User_test.dict') :
        user, ads = line.strip().split('\x01')
        userSet.add(user)
    
    dumpUserRawFeatureGivenUserSet(input_file, userSet, TMP_DATA_DIR_PATH + 'userRawFeature_test.dict')
    dumpAd2UserStatus(input_file, adSet, userSet, TMP_DATA_DIR_PATH + 'ad2userStatus_test.dict') 
...
def Prediction() :
    ...
    features = TMP_DATA_DIR_PATH + 'finalData4SVMRanking_test.dat'
    model = TMP_DATA_DIR_PATH + 'SVMRanking.model'
    predictions = TMP_DATA_DIR_PATH + 'SVMRanking_test.prediction'
