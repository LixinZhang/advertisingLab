...
class TrainingSet_util :
    ...
    @staticmethod
    def parse_training_log_file (line) :
        terms = line.strip().split('\t')
        if len(terms) != 12 : return None
        Click, Impression, Display_url,\
                AdID, AdvertiserID, Depth, \
    ...
    @staticmethod
    def filterUsers(originFile = '', filterResultFile = '', usersetFile = '',  adSetFile = '') :
        userset = set()
        for line in file(usersetFile) :
            userset.add(line.strip())

        adset = set()
        for line in file(adSetFile) :
            adset.add(line.strip())

        filterResult = file(filterResultFile, 'w')

        line_cnt = 0
        for line in file(originFile) :
            line_cnt += 1
            terms = TrainingSet_util.parse_training_log_file(line) 
            if terms == None : continue
            Click, Impression, Display_url,\
                AdID, AdvertiserID, Depth, \
                Position, QueryID, KeywordID,\
    ...
    @staticmethod
    def DumpUserID2AdvIDPair(filename, outputfile) :
        outputfile = file(outputfile, 'w')
        output_format = '%s\t%s\t%s\t%f\n'
        UserAd_click = {}
        UserAd_Impression = {}
        total_click = 0
        total_impression = 0
        for line in file(filename) :
            ...
            if UserID not in UserAd_click :
                UserAd_click[UserID] = {}
                UserAd_Impression[UserID] = {}

            if AdID not in UserAd_click[UserID] :
                UserAd_click[UserID][AdID] = 0
                UserAd_Impression[UserID][AdID] = 0

            UserAd_click[UserID][AdID] += Click
            UserAd_Impression[UserID][AdID] += Impression
            total_click += Click
            total_impression += Impression

        alpha, beta = total_click * 1.0 / len(UserAd_click), total_impression * 1.0 / len(UserAd_click)
        print alpha, beta, len(UserAd_click), Click, Impression

        for UserID in UserAd_click :
            for AdID in UserAd_click[UserID] :
                Click = UserAd_click[UserID][AdID]
                Impression = UserAd_Impression[UserID][AdID]
                if Click * 3 > Impression :
                    outputfile.write(output_format % (UserID, AdID, '3', (Click * 1.0 + alpha) / (beta + Impression)))
                elif Click > 0 :
                    outputfile.write(output_format % (UserID, AdID, '2', (Click * 1.0 + alpha) / (beta + Impression)))
                else :
                    outputfile.write(output_format % (UserID, AdID, '1', (Click * 1.0 + alpha) / (beta + Impression)))
        outputfile.close()
...
#    ts_util.prepare()
#    ts_util.printStatus()
#    ts_util.aggregateUser('./test_user.filter', './test_user.aggr')
#    ts_util.filterAdditionalData('user.dat')
#    ts_util.expandId2Tokens()
    ts_util.expandId2Tokens('./test_user.aggr', './test_user.expand2Tokens', False)
#    TrainingSet_util.filterUsers('../../dataset/test.txt', '../test.dat.filter', '../user.dat', '../adID.dat')
    #TrainingSet_util.DumpUserID2AdvIDPair('../user.filter', 'userID2AdIDFlag') 
    TrainingSet_util.DumpUserID2AdvIDPair('./test_user.filter', 'test_userID2AdIDFlag') 
